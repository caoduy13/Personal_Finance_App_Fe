...
function OnboardingPage() {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      const u = result?.user
      if (u && typeof u === 'object' && result?.success !== false) {
        setUser(u)
      ...
      const msg = err?.response?.data?.message || err?.message || ONBOARDING_UI.submitError
  ...
  return (
    <PageWrapper>
      <div style={pageBg}>
        ...
        {submitError ? (
          <p style={{ ...caption, color: '#b91c1c', marginBottom: 8 }}>{submitError}</p>
        ) : null}
        <div style={stepBar}>
